criaCartao(
    'Supernatural',
    'Qual é o nome dos dois irmãos protagonistas da série?',
    'Sam e Dean Winchester.'
)

criaCartao(
    'Supernatural',
    'Qual é a marca e o ano do famoso carro preto conduzido por Dean?',
    'Chevrolet Impala 1967.'
)

criaCartao(
    'Supernatural',
    'Qual é a icônica frase que resume o "negócio da família" dos Winchester?',
    'Salvar pessoas, caçar coisas, o negócio da família.'
)

criaCartao(
    'Supernatural',
    'Como se chama o anjo que se torna um dos aliados mais importantes dos irmãos a partir da 4.ª temporada?',
    'Castiel.'
)

criaCartao(
    'Supernatural',
    'Qual é o nome do demônio de olhos amarelos que foi o principal antagonista nas primeiras temporadas e responsável pela morte da mãe deles?',
    'Azazel.'
)

criaCartao(
    'Supernatural',
    'Que substância comum no dia a dia é usada pelos caçadores para afastar fantasmas e demônios e fazer barreiras?',
    'Sal.'
)

criaCartao(
    'Supernatural',
    'Como se chama o rei do Inferno de temperamento sarcástico que frequentemente oscila entre aliado e inimigo dos irmãos?',
    'Crowley.'
)

criaCartao(
    'Supernatural',
    'Qual é o nome do amigável caçador veterano que serve como uma figura paterna e mentor para Sam e Dean?',
    'Bobby Singer.'
)

criaCartao(
    'Supernatural',
    'Que música clássica do Kansas toca sempre no início de cada final de temporada "The Road So Far"?',
    'Carry On Wayward Son.'
)

